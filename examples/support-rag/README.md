...
a customer question on stdin and returns a canned, policy-grounded answer. It
uses no model and no network, so it is deterministic in CI. Each case carries
the policy chunk it should be grounded in as `context`, exactly as a real RAG
app would retrieve it.
...
Exit code `0`: every answer cited a policy, none made a forbidden promise, and
the run cleared its 95% pass-rate floor.
