...
- **GitHub Action:** bump the pinned tag (`eval-core/evalcore@v0.7.5`), or set
...
- `html-artifact` (optional) uploads a self-contained HTML report as a build
  artifact with this name, even when the run fails; defaults to
  `evalcore-report`. Set it to `""` to disable the upload.
