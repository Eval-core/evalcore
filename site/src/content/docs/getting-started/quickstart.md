---
title: Quickstart
description: Grade a real support bot against its policy context in five minutes — no API keys, no network. Run the shipped suite, read every line, add an HTML report, then swap in your own app.
---
...
suite-level compliance gate held — all offline, in a few milliseconds.
...
- **`targets`** — what's evaluated. `shell` runs a command with each case's
...
- **`scorers`** — how each answer is judged. `contains` passes when the output
  ...
  scorer passes.
- **`run.gates`** — a floor over the whole suite. `pass_rate: 0.95` means at
...
- **`PASS late-refund (12ms)`** — the case id, then the target's latency in
  milliseconds. Every passing case is one line.
- **`4 passed, 0 failed, 4 total`** — the summary line.
- **`GATE PASS pass_rate >= 0.95 (actual 1.00)`** — every gate prints its
  own line with the threshold and the actual value, so a `GATE FAIL` tells you
  exactly how far short the run fell.
...
That is one file — no assets, no server — you can open in a browser or attach to
a PR. It shows the pass/fail summary, the gate outcomes, and every case's
answer with its per-scorer scores expandable inline. In CI, the same report is
the audit artifact a reviewer clicks straight from the pull request. More in
...
call is served from the recording — no network, no API key, no flaky judge — so
even LLM-graded suites run offline, deterministically, and for $0. That is what
turns an eval suite into a blocking CI check. See
