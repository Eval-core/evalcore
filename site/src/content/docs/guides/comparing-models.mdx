...
description: Run one suite against several targets in a single invocation for a side-by-side comparison with per-case winners, per-target cost, and one CI exit code.
---
...
<CastFrame
	cast="matrix"
	title="evalcore run evals.yaml --matrix good,bad"
	...
	caption="One invocation, two targets, one comparison, with per-case winners and a win/tie tally."
/>
