...
description: "Run every case N times and aggregate: trials, require policies, how verdicts and scores fold together, and the determinism story."
---
...
<CastFrame
	cast="trials"
	title="evalcore run site/demo/trials/evals.yaml"
	...
	caption="Run every case N times, with each verdict tagged by its passing-trial count."
/>
...
:::note[Token totals are trial-aware]
With `run.trials` greater than 1, both the **cost** and the **token** totals in
the terminal and HTML summaries sum every trial, and every priced scorer, so the
figures are exact multi-trial totals. Gating on cost or budget and reading the
token count both reflect the full N trials.
:::
