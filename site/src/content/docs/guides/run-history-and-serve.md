...
**Run history** does exactly that. Every run appends a row to your local store,
and `evalcore serve` opens a small read-only web viewer over those rows — a
listing with a trend sparkline, each run's full HTML report, and a diff between
any two runs. It is entirely local: no server to sign up for, no data leaving
your machine.

## Recording history

History is **on by default**. Every `evalcore run` appends one row per executed
run to the store, capturing that run's full result — the same cases, scores,
gates, and classification the report shows. You don't have to do anything:
...
[record/replay cache](/evalcore/guides/record-replay/) and baselines — one local
store per project, in a new `run_history` table that sits beside the existing
ones (adding it doesn't touch the cache). Nothing is written anywhere else.
...
machine. Your call — history is additive either way.
...
The viewer is **local-first and read-only, by design**. It binds `127.0.0.1`
only — localhost, never a wildcard address — and that is the *entire* security
model: there is **no auth because there is no remote access**. Every route is a
`GET` (any other method is a `405`); there are **zero mutation endpoints** — the
viewer only ever reads the store. No telemetry, no external requests, no
JavaScript frameworks; the pages are self-contained HTML. **Nothing leaves your
machine.**

This is the same principle stated in the [FAQ](/evalcore/faq/): EvalCore is
local-first and CI-native — no server, no signup, results in a SQLite file next
to your repo. A hosted tier, if one ever exists, composes *with* that; it will
never gate the local features. Run history and the viewer work fully offline,
forever, with no account.
...
table, an inline **pass-rate sparkline** plots the trend over the most recent
runs — a compact glyph for "are we getting better or worse?"

A real listing looks like this (four runs — two single-target, then a
...
Clicking a run id opens that run's full detail — and it is **byte-for-byte the
same document `--html` writes**: the header counts, the gates panel, one
expandable row per case with the output, per-scorer scores, trials detail, and
agent trajectory. The viewer reuses the [HTML report](/evalcore/guides/html-reports/)
renderer verbatim, so a page you serve here is identical to a report you'd attach
to a PR. (An unknown id is a plain 404.)
...
- **Model A vs model B** — diff `gpt`'s run against `claude`'s run, even though
  you ran them separately on different days.
- **Yesterday vs today** — diff two runs of the *same* target to see exactly
  which cases moved.
