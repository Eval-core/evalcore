...
that answers a case," the two things you compare can be two models, two prompts,
or two deployed endpoints. Anything you can configure as a target works.
...
<figure class="ec-cast">
	...
	<figcaption>One invocation, two targets, one comparison, with per-case winners and a win/tie tally.</figcaption>
</figure>
...
Or select them on the command line with a comma list that overrides
`run.matrix`:
...
The winner of a case is the arm with the strictly highest mean case score, which
is the mean of that case's scorer values for that arm. It is *not* PASS-vs-FAIL:
two arms can both PASS and still have a winner if one scored higher (say a
`judge` or `similarity` scorer gave 0.9 vs 0.7).
...
- A case where no arm produced a score (every arm errored, or the budget
  skipped it) is also a tie, because there is nothing to compare.
...
A matrix run exits `0` iff every arm satisfies today's whole contract, meaning
all of that arm's cases pass *and* every configured gate holds. Otherwise it
exits `1`.
