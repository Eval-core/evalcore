...
stay under a step budget? EvalCore evaluates the run your agent already emitted
as telemetry: it ingests recorded traces and grades the answer and the path in
one suite. There is no SDK to install and no integration to write, so any
language or framework works.
...
Run the shipped example. It works offline, with no network access:
...
EvalCore reads one trace per case as a single JSON file. You produce those
...
Only tool-call spans become trajectory steps; LLM and other spans are not steps,
...
If you'd rather emit EvalCore's format directly, without any OTel plumbing,
write the canonical JSON: a `steps` array plus an optional `final_output`:
...
The `trajectory` scorer always operates on the steps. It passes only if every
rule holds; each failed rule contributes a reason naming the offending step.
There are three rule types. Here they are with realistic policy examples:
...
- `must_call: T` holds iff at least one step calls `T` and satisfies every `with`
  ...
  call, and it **fails** if `U` never ran.
...
- OTel export: the final answer is read from the root span, using
...
"Was the answer right *and* was the path safe" becomes one suite with one exit
code. If a trace carries no final answer, the text output stays the serialized
trajectory JSON, so older suites that grade it with `contains` keep working.
...
The shipped `examples/agent-trace/` suite grades two cases, one native and one
OTel, against the same scorers:
