#!/usr/bin/env python3
# EvalCore subprocess scorer shim: DeepEval "contextual recall"
...
#
# Contextual recall measures whether the retrieval context supports the
# expected output. It requires per-case `context` AND `expected` (the expected
# output / ground truth).
#
...
def score_with_deepeval(case):
    ...
    expected_output=expected, retrieval_context=context (all required by this
    metric).
    """
...
def main(argv):
    ...
    if not isinstance(case.get("expected"), str) or not case["expected"]:
        print(
            f'{LIBRARY}/{METRIC}: this metric requires an expected output — '
