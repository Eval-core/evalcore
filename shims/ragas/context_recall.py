#!/usr/bin/env python3
# EvalCore subprocess scorer shim: Ragas "context recall" (LLMContextRecall).
#
...
#
# Context recall measures whether the retrieved context covers the ground
# truth. It requires per-case `context` AND `expected` (the ground truth /
# reference).
#
...
def score_with_ragas(case):
    ...
    retrieved_contexts=context, reference=expected (required by this metric).
    """
    import asyncio
...
def main(argv):
    ...
    if check:
        ...
        # calling an LLM. Metric preconditions (like requiring `context` and
        # `expected`) are intentionally NOT enforced here — check mode
        # validates the protocol, not a metric's inputs — so CI can smoke-test
        # offline.
    ...
    if not isinstance(case.get("expected"), str) or not case["expected"]:
        print(
            f'{LIBRARY}/{METRIC}: this metric requires a ground truth — '
            'set "expected" on your dataset cases',
            file=sys.stderr,
        )
        return 1
